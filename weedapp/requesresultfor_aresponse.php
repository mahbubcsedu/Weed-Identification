<?php
    ...
    $expert_response_id = $_REQUEST["expert_response_id"];
	//$identificationid = $_REQUEST["identification_id"];
	require_once('db.inc.php');

	$query="select *from weeds,rank where weeds.weed_id=rank.weed_id and expert_response_id=$expert_response_id";
	...
	if($result && mysql_numrows($result)>0)
	{
		...
		while ($db_locations = mysql_fetch_assoc($result))
		{
			...
			$weed = array('weed_id' => $weedid, 'common_name' => $commonname, 'latin_name' => $latinname, 'weed_type' => $weedtype, 'life_cycle' => $lifecycle,'rank' => $rank, 'season' => $season, 'site' => $site, 'prof_control' => $profcontrol, 'home_control' => $homecontrol, 'image_url' => $image_url);
